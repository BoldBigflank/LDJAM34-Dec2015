...
namespace FluffyUnderware.Curvy
{
    ...
    public class CurvySpline : CurvySplineBase
    {
        ...
        public override Vector3 InterpolateUserValue(float tf, int index)
        {
            float localF;
            CurvySplineSegment seg = TFToSegment(tf, out localF);
            return (seg) ? seg.InterpolateUserValue(localF, index) : Vector3.zero;
        }
        ...
        public CurvySplineSegment[] Add(params Vector3[] controlPoints)
        {
            if (!OnBeforeControlPointAddEvent(new CurvyControlPointEventArgs(this)).Cancel)
            {
                ...
                for (int i = 0; i < controlPoints.Length; i++)
                    OnAfterControlPointAddEvent(new CurvyControlPointEventArgs(this,this,cps[i],CurvyControlPointEventArgs.AddMode.After));
        ...
        public CurvySplineSegment InsertBefore(CurvySplineSegment controlPoint)
        {
            if (!OnBeforeControlPointAddEvent(new CurvyControlPointEventArgs(this,this,controlPoint,CurvyControlPointEventArgs.AddMode.Before)).Cancel)
            {
                ...
                go.layer = CurvyGlobalManager.SplineLayer;
        ...
        public CurvySplineSegment InsertAfter(CurvySplineSegment controlPoint)
        {
            if (!OnBeforeControlPointAddEvent(new CurvyControlPointEventArgs(this,this,controlPoint,CurvyControlPointEventArgs.AddMode.After)).Cancel)
            {
                ...
                var e = new CurvyControlPointEventArgs(this,this, cp,CurvyControlPointEventArgs.AddMode.After);
                OnAfterControlPointAddEvent(e);
                OnAfterControlPointChangesEvent(e);
                return cp;
            }
        ...
        public override void Clear()
        {
            ...
            {
                if (OnBeforeControlPointDeleteEvent(new CurvyControlPointEventArgs(this,this, seg)).Cancel)
                    return;
            }
        ...
        /// <summary>
        /// Same like Move(), but fires OnMoveControlPointReached event
        /// </summary>
        Vector3 eventAwareMove(ref float tf, ref int direction, float fDistance, CurvyClamping clamping, bool fastMode)
        {
            ...
            var e = new CurvySplineMoveEventArgs(this, this, seg, tf, fDistance, direction);
        ...
        Vector3 eventAwareMoveDistance(ref float tf, ref int direction, float distance, CurvyClamping clamping, bool fastMode)
        {
            var seg = TFToSegment(tf);
            float tfDist;
            var e = new CurvySplineMoveEventArgs(this, this, seg, tf, distance, direction,true);
        ...
        public void InsertUserValueAt(int index)
        {
            if (index >= 0 && index < UserValueSize)
                for (int i = 0; i < ControlPointCount; i++)
                    ControlPoints[i].UserValues = ControlPoints[i].UserValues.InsertAt(index);
        ...
        public void RemoveUserValueAt(int index)
        {
            if (UserValueSize > 0)
            {
                ...
                for (int i = 0; i < ControlPointCount; i++)
                    ControlPoints[i].UserValues = ControlPoints[i].UserValues.RemoveAt(index);
                UserValueSize--;
            }
        }

        [System.Obsolete("Use CurvySpline.CatmullRom() instead!")]
        public static Vector3 CatmulRom(Vector3 T0, Vector3 P0, Vector3 P1, Vector3 T1, float f)
        {
            return CatmullRom(T0, P0, P1, T1, f);
        }

        [System.Obsolete("Use CurvySplineSegment.NextControlPoint instead!")]
        public CurvySplineSegment NextControlPoint(CurvySplineSegment controlpoint)
        {
            return (controlpoint) ? controlpoint.NextControlPoint : null;
        }
        [System.Obsolete("Use CurvySplineSegment.PreviousControlPoint instead!")]
        public CurvySplineSegment PreviousControlPoint(CurvySplineSegment controlpoint)
        {
            return (controlpoint) ? controlpoint.PreviousControlPoint : null;
        }

        [System.Obsolete("Use CurvySplineSegment.NextSegment instead!")]
        public CurvySplineSegment NextSegment(CurvySplineSegment segment)
        {
            return (segment) ? segment.NextSegment : null;
        }

        [System.Obsolete("Use CurvySplineSegment.PreviousSegment instead!")]
        public CurvySplineSegment PreviousSegment(CurvySplineSegment segment)
        {
            return (segment) ? segment.PreviousSegment : null;
        }

        [System.Obsolete("Use CurvySplineSegment.NextTransform instead!")]
        public Transform NextTransform(CurvySplineSegment controlpoint)
        {
            return (controlpoint) ? controlpoint.NextTransform : null;
        }

        [System.Obsolete("Use CurvySplineSegment.PreviousTransform instead!")]
        public Transform PreviousTransform(CurvySplineSegment controlpoint)
        {
            return (controlpoint) ? controlpoint.PreviousTransform : null;
        }

        [System.Obsolete("Use CurvySpline.InsertAfter() instead!")]
        ...
        [System.Obsolete("Use CurvySpline.InsertBefore() instead!")]
        public CurvySplineSegment Add(bool refresh, CurvySplineSegment insertBefore)
        {
            return InsertBefore(insertBefore);
        }

        [System.Obsolete("Use CacheDensity instead")]
        public int Granularity { get { return CacheDensity; } set { CacheDensity = value; } }

        [SerializeField, FormerlySerializedAs("UserValueSize"), HideInInspector]
        int m_UserValueSize = 0;
        ...
        {
            get { return m_UserValueSize; }
            set
            {
                int v= Mathf.Max(0, value);
                if (m_UserValueSize != v)
                {
                    m_UserValueSize = v;
                }
                for (int i = 0; i < ControlPointCount; i++)
        ...
        [System.Obsolete]
        CurvyInitialUpDefinition InitialUpVector;
