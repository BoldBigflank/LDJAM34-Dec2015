    ...
    public static class DTToolbar
    {
        ...
        static void loadItems()
        {
            var types = (from asm in AppDomain.CurrentDomain.GetAssemblies()
                         ...
                         where t.IsSubclassOf(typeof(DTToolbarItem))
                            let attribs = t.GetCustomAttributes(typeof(ToolbarItemAttribute),true)
        ...
        {
            foreach (var project in DT.Projects)
                foreach (var item in project.ToolbarItems)
        ...
        {
            var v = GUIUtility.GUIToScreenPoint(Vector2.zero);
            var r=SceneView.currentDrawingSceneView.position;
        ...
        static void calcMaxItemDimension()
        {
            ...
            foreach (var prj in DT.Projects)
            {
                ...
                foreach (var item in prj.ToolbarItems)
                    if (item.Visible)
